...
class Parser:
    ...
    def expr(self):
        ...
        if self.term() and self.__expect__(TT_ADD) and self.expr():
            return True
    ...
    def term(self):
        pos = self.__current_pos__
        if self.__expect__(TT_USUB) and self.factor():
            return True
        self.__reset_pos__(pos)
    ...
    def parse(self):
        if self.expr() and self.input[self.__current_pos__].type == TT_EOF:
            return True
        else:
            return False
